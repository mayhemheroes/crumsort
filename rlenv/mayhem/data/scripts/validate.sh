...
if [ "$FIRST_CRASHED_ANY" = true ]; then
    ...
    echo "Initial triage: PASSED all 20 attempts (MTV exit code: $FIRST_TRIAGE_EXIT_CODE)"
fi
...
if [ "$SECOND_CRASHED_ANY" = true ]; then
    ...
    echo "Second triage: PASSED all 20 attempts (MTV exit code: $SECOND_TRIAGE_EXIT_CODE)"
fi
